...
class TestTweetInput:
    ...
    def test_tweet_input_with_message(self):
        """Test creating TweetInput with a message."""
        tweet_input = TweetInput(action="Hello from robot!")
        assert tweet_input.action == "Hello from robot!"

    def test_tweet_input_with_emoji(self):
        """Test creating TweetInput with emoji."""
        tweet_input = TweetInput(action="Hello! 🤖")
        assert tweet_input.action == "Hello! 🤖"

    def test_tweet_input_max_length(self):
        """Test creating TweetInput with 280 characters."""
        long_text = "a" * 280
        tweet_input = TweetInput(action=long_text)
...
class TestTweet:
    ...
    def test_tweet_different_input_output(self):
        ...
        input_msg = TweetInput(action="Input message")
        output_msg = TweetInput(action="Output message")
        tweet = Tweet(input=input_msg, output=output_msg)
        assert tweet.input.action == "Input message"
        assert tweet.output.action == "Output message"
