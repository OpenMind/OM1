...
def test_posture_classification_good():
    """Test classification of good posture."""
    input_data = PostureDetectionInput(
        posture_type=PostureType.GOOD,
        severity=PostureSeverity.MILD
    )
    assert input_data.posture_type == PostureType.GOOD
    # Good posture should have mild severity
    assert input_data.severity == PostureSeverity.MILD


def test_posture_classification_poor_postures():
    """Test classification of poor postures."""
    poor_postures = [
        PostureType.SLUMPED,
        PostureType.HUNCHED,
        PostureType.LEANING,
    ...
    for posture_type in poor_postures:
        ...
        assert input_data.posture_type == posture_type
        assert input_data.posture_type != PostureType.GOOD


def test_posture_classification_laying():
    """Test classification of laying posture."""
    input_data = PostureDetectionInput(
        posture_type=PostureType.LAYING,
        severity=PostureSeverity.MILD
    )
    assert input_data.posture_type == PostureType.LAYING
    # Laying may indicate fatigue but is not necessarily poor posture
    assert input_data.severity in [PostureSeverity.MILD, PostureSeverity.MODERATE, PostureSeverity.SEVERE]
