...
def mock_decorators():
    ...
    with (
        ...
        patch("llm.plugins.dual_llm.LLMHistoryManager") as mock_history_manager,
    ):
        yield
...
def dual_llm(mock_llm_classes):
    config = DualLLMConfig(
        local_llm_type="MockLocal",
        cloud_llm_type="MockCloud",
        api_key="test_key"
    )
    # Mock openai.AsyncClient used in __init__
    with patch("openai.AsyncClient"):
        llm = DualLLM(config)
    
    # Inject our mock instances
...
async def test_race_cloud_wins(dual_llm):
    ...
    cloud_result = CortexOutputModel(thought="Cloud", actions=[])
    
    async def slow_local(*args):
        await asyncio.sleep(0.5)
        return CortexOutputModel(thought="Local", actions=[])
...
async def test_race_both_fast_local_has_function(dual_llm):
    ...
    local_result = CortexOutputModel(
        thought="Local", 
        actions=[Action(type="func", value="val")]
    )
    cloud_result = CortexOutputModel(thought="Cloud", actions=[])
    
    async def fast(*args): return None # Unused for mocked return_value
    
    dual_llm._local_llm.ask = AsyncMock(return_value=local_result)
    dual_llm._cloud_llm.ask = AsyncMock(return_value=cloud_result)
    
    dual_llm.TIMEOUT_THRESHOLD = 1.0 # Large enough for both
...
async def test_race_both_fast_cloud_has_function(dual_llm):
    ...
    local_result = CortexOutputModel(thought="Local", actions=[])
    cloud_result = CortexOutputModel(
        thought="Cloud", 
        actions=[Action(type="func", value="val")]
    )
...
async def test_race_both_fast_both_functions_eval(dual_llm):
    ...
    local_result = CortexOutputModel(thought="Local", actions=[Action(type="f", value="v")])
    cloud_result = CortexOutputModel(thought="Cloud", actions=[Action(type="f", value="v")])
    
    dual_llm._local_llm.ask = AsyncMock(return_value=local_result)
    dual_llm._cloud_llm.ask = AsyncMock(return_value=cloud_result)
    dual_llm.TIMEOUT_THRESHOLD = 1.0
    
    # Mock eval client response
    mock_eval_response = MagicMock()
    mock_eval_response.choices = [
        MagicMock(message=MagicMock(content="B")) # B maps to Cloud
    ]
...
async def test_timeout_both_slow(dual_llm):
    ...
    # Local takes 0.2s, Cloud takes 0.3s. Timeout is 0.1s.
    # Logic should trigger "Neither in time" path and wait for Local.
    ...
    # We need to increase test timeout or ensuring logic waits
    response = await dual_llm.ask("test prompt")
