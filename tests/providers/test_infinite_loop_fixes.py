...
class TestInfiniteLoopFixes:
    ...
    def test_odom_provider_stop_event(self):
        ...
        provider = OdomProvider()
        
        # Start the provider
        provider.start()
        
        # Verify the thread is running
        assert provider._odom_processor_thread is not None
        assert provider._odom_processor_thread.is_alive()
