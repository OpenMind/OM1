...
def test_init_signature(simulator_class: Type[Simulator]):
    ...
    assert base_params == impl_params, f"{simulator_class.__name__} __init__ signature mismatch"
...
def test_tick_signature(simulator_class: Type[Simulator]):
    ...
    assert base_params == impl_params, f"{simulator_class.__name__} tick signature mismatch"
...
def test_sim_signature(simulator_class: Type[Simulator]):
    ...
    assert base_params == impl_params, f"{simulator_class.__name__} sim signature mismatch"
...
def test_return_type_annotations(simulator_class: Type[Simulator]):
    ...
    for method_name in methods_to_check:
        ...
        impl_return = simulator_class.__dict__[method_name].__annotations__.get("return")
...
def test_docstring_exists(simulator_class: Type[Simulator]):
    ...
    assert simulator_class.__doc__ is not None, f"{simulator_class.__name__} missing class docstring"

    methods_to_check = ["tick", "sim"]
    for method_name in methods_to_check:
        method = getattr(simulator_class, method_name)
        assert method.__doc__ is not None, f"{simulator_class.__name__}.{method_name} missing docstring"
