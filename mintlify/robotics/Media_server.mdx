...
> **Note:** We use the `bluenviron/mediamtx:latest-ffmpeg` Docker image, which includes FFmpeg built-in. This enables efficient video encoding, transcoding, and streaming capabilities directly within the media server, eliminating the need for separate encoding infrastructure.
