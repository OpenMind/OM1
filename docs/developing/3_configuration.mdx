...
OM1 runtime applies strict validation when updating or loading configuration files.
This is designed to prevent invalid or incompatible configurations from reaching
production systems.

The validation pipeline includes:

- **JSON5 syntax validation**
  Ensures the configuration file is valid JSON5 and can be safely parsed.

- **Schema validation (single-mode / multi-mode)**
  The runtime automatically detects whether the configuration is single-mode or
  multi-mode and validates it against the corresponding schema.

  Schema files:
  - `config/schema/single_mode_schema.json`
  - `config/schema/multi_mode_schema.json`

- **Runtime version compatibility checks**
  The `version` field is validated against the currently running runtime version to
  prevent incompatible configuration updates.

- **Atomic writes with backup & restore**
  Configuration updates are written atomically:
  - Existing config files are backed up before writing
  - Temporary files are used during updates
  - If any write error occurs, the previous configuration is restored automatically

When validation fails, the runtime provides **clear and actionable error messages**
that include the exact validation stage and the schema file involved, making
misconfigurations easy to diagnose and fix.
