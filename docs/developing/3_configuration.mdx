...
OM1 can act as both an **MCP client** (using external MCP tools) and an **MCP server** (exposing OM1 actions).
...
Notes:
- `clients` are external MCP servers OM1 can call as tools.
- `server` exposes OM1 actions via MCP (tools: `om1_actions`, `om1_action`).
- Keep `tool_prefix` as `mcp` so tool arguments are preserved correctly.
