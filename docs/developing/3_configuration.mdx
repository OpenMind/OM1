...
* **version** Defines runtime versioning support and verification across both single-mode and multi-mode configuration.
...
All integration tests now include "version": "v1.0.0" so that test configs remain versioned and validated.

A dedicated test suite (tests/runtime/test_version.py) covers:
...
As part of introducing versioning, the unused `get_nested_value helper` was removed from
...
To introduce multimode config, refer [introduce new mode]()
