...
- ROS topic timestamps may lag behind system time.
- Disable automatic date/time and manually sync:
  ```bash
  sudo date -s "@unix"
  ```
- Find robot timestamp:
  ```bash
  ros2 topic echo /utlidar/robot_pose --field header.stamp
  ```
- Share internet connection for auto-sync:
  ```bash
...
- [Unitree Go2 Documentation](https://www.unitree.com/products/go2)
