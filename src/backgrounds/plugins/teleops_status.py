...
from providers.teleops_status_provider import (
    BatteryStatus,
    TeleopsStatus as TeleopsStatusData,
    TeleopsStatusProvider,
)
from providers.config_provider import ConfigProvider


class TeleopsStatusConfig(BackgroundConfig):
    """
...
class TeleopsStatus(Background[TeleopsStatusConfig]):
    """
    Teleops Status Background Plugin.
    Sends periodic teleops status updates so device appears on dashboard.
    Works on any platform (Mac, Linux, Windows).
    """

    def __init__(self, config: TeleopsStatusConfig):
        ...
        if not api_key:
            logging.error("API key not found for TeleopsStatus")
            return
        ...
        self.thread.start()
        logging.info(f"TeleopsStatus started - sending status every {self.update_interval}s")

    def _status_loop(self):
        """Background loop to send status updates."""
        while self.running:
            try:
                status = TeleopsStatusData(
                ...
                logging.debug(f"TeleopsStatus: sent status update for {self.machine_name}")
            except Exception as e:
                logging.error(f"TeleopsStatus error: {e}")
    ...
    def stop(self):
        ...
        self.running = False
        logging.info("TeleopsStatus stopped")
