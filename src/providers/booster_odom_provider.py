...
class BoosterOdomProvider:
    ...
    def __init__(self, topic: str = "odometer_state"):
        ...
        topic : str
            ...
            Defaults to "odometer_state".
        """
        logging.info("Booting Booster Odom Provider")
    ...
    def start(self) -> None:
        ...
        if self._odom_reader_thread and self._odom_reader_thread.is_alive():
            ...
            if not self.topic:
                logging.error(
                    "Topic must be specified to start the Booster Odom Provider."
                )
                return
    ...
    def process_odom(self):
        ...
        while not self._stop_event.is_set():
            ...
            logging.debug(
                f"booster odom: X:{self.x} Y:{self.y} W:{self.odom_yaw_m180_p180} H:{self.odom_yaw_0_360} T:{self.odom_rockchip_ts}"
            )
    ...
    def stop(self):
        """
        Stop the BoosterOdomProvider and clean up resources.
        """
        ...
        if self._odom_reader_thread:
            ...
            logging.info("BoosterOdomProvider reader thread stopped.")

        if self._odom_processor_thread:
            self._odom_processor_thread.join()
            logging.info("BoosterOdomProvider processor thread stopped.")
