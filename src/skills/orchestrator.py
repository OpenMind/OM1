...
class SkillOrchestrator:
    ...
    and provides the ``read_skill`` function schema.
    ...
    def __init__(self, skill_names: List[str]) -> None:
        ...
        # Load only requested skills
        self._skills = {name: entry for name, entry in loader.skills.items() if name in skill_names}
        ...
        if not self._skills:
            ...
            logging.info(
                f"SkillOrchestrator initialized with " f"{len(self._skills)} skill(s): {list(self._skills.keys())}"
            )

    def get_tool_schema(self) -> Optional[dict]:
        """Get the combined tool schema for all loaded skills.

        Returns
        -------
            The tool schema dict, or None if no skills are loaded.
        """
        if not self._skills:
            return None
        return build_tool_schema(self._skills)
    ...
    async def process(
        ...
        for round_idx in range(max_rounds):
            ...
            new_actions = [a for a in skill_actions if self._call_signature(a) not in succeeded_calls]
            if not new_actions:
                break

            # Dispatch OM1 actions
            om1_only_actions = [
                a for a in output.actions if a.type != READ_SKILL_TOOL and not a.type.startswith("mcp_")
            ]
            if om1_only_actions and dispatch_om1:
                await dispatch_om1(om1_only_actions)

            output.actions = [a for a in output.actions if a.type == READ_SKILL_TOOL or a.type.startswith("mcp_")]

            logging.info(
                f"Skill round {round_idx + 1}/{max_rounds}: " f"executing {len(new_actions)} read_skill call(s)"
            )
    ...
    def execute_read_skills(self, actions: List[Any], succeeded_calls: Set[str]) -> List[dict]:
    ...
    def build_skill_recall_prompt(self, original_prompt: str, results: List[dict]) -> str:
    ...
    def _execute_read_skill(self, action: Any) -> dict:
        ...
        try:
            ...
            if not skill:
                ...
                content = f"Error: Skill '{skill_name}' not found. " f"Available skills: {available}"
                return {
