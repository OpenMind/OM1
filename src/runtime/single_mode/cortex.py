...
class CortexRuntime:
    ...
    async def reload_from_patch(
        self, changed_fields: set[str], new_raw: dict[str, Any]
    ) -> None:
        whitelist = {
            "system_prompt_base",
            "system_governance",
        ...
        if changed_fields.issubset(whitelist):
            if "system_prompt_base" in new_raw:
                self.config.system_prompt_base = new_raw["system_prompt_base"]
            if "system_governance" in new_raw:
                self.config.system_governance = new_raw["system_governance"]
            if "system_prompt_examples" in new_raw:
                self.config.system_prompt_examples = new_raw["system_prompt_examples"]
            self._last_raw_config = new_raw
            logging.info(
                f"Applied partial hot-reload for fields: {sorted(changed_fields)}"
            )
            return
    ...
    async def _reload_config(self) -> None:
        ...
        try:
            ...
            changed_fields = self._compute_patch(self._last_raw_config, new_raw)
            if not changed_fields:
                self._last_raw_config = new_raw
                return

            await self.reload_from_patch(changed_fields, new_raw)
