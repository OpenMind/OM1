...
def cortex_runtime(mock_system_config):
    ...
    with (
        ...
        runtime = ModeCortexRuntime(mock_system_config, "test_config", use_watchdog=False)
...
class TestModeCortexRuntimeHotReload:
    ...
    def test_get_file_mtime_existing_file(self, mock_system_config, temp_config_file):
        ...
        with (
            ...
            runtime = ModeCortexRuntime(
                mock_system_config, "test_config", hot_reload=True, use_watchdog=False
            )
    ...
    @pytest.mark.asyncio
    async def test_partial_hot_reload_prompts(self, mock_system_config):
        with (
            patch("runtime.multi_mode.cortex.ModeManager") as mock_manager_class,
            patch("runtime.multi_mode.cortex.IOProvider"),
            patch("runtime.multi_mode.cortex.SleepTickerProvider"),
            ...
            runtime.use_watchdog = False
            runtime.mode_config.system_governance = "old_gov"
            runtime.mode_config.system_prompt_examples = "old_examples"
            runtime.mode_config.modes["default"].system_prompt_base = "old_base"

            runtime.current_config = Mock()
            runtime.current_config.system_governance = "old_gov"
            runtime.current_config.system_prompt_examples = "old_examples"
            runtime.current_config.system_prompt_base = "old_base"

            runtime.config_path = "/tmp/config.json5"

            old_raw = {
                "system_governance": "old_gov",
                "system_prompt_examples": "old_examples",
                "modes": {
                    "default": {"system_prompt_base": "old_base"},
                    "advanced": {"system_prompt_base": "advanced_base"},
                },
            }
            raw = {
                "system_governance": "new_gov",
                "system_prompt_examples": "new_examples",
                "modes": {
                    "default": {"system_prompt_base": "new_base"},
                    "advanced": {"system_prompt_base": "advanced_base"},
                },
            }
            ...
            assert runtime.mode_config.system_governance == "new_gov"
            assert runtime.mode_config.system_prompt_examples == "new_examples"
            assert runtime.mode_config.modes["default"].system_prompt_base == "new_base"
            assert runtime.current_config.system_governance == "new_gov"
            assert runtime.current_config.system_prompt_examples == "new_examples"
            assert runtime.current_config.system_prompt_base == "new_base"
    ...
    async def test_reload_config_success(self, mock_system_config):
        ...
        with (
            ...
            raw = {"system_governance": "new_gov", "modes": {"test_mode": {}}}
            with patch("builtins.open", mock_open(read_data=json.dumps(raw))):
                await runtime._reload_config()
    ...
    async def test_reload_config_mode_not_found(self, mock_system_config):
        ...
        with (
            ...
            raw = {"system_governance": "new_gov", "modes": {"old_mode": {}}}
            with patch("builtins.open", mock_open(read_data=json.dumps(raw))):
                await runtime._reload_config()
    ...
    async def test_reload_config_failure(self, mock_system_config):
        ...
        with (
            ...
            raw = {"system_governance": "new_gov", "modes": {"default": {}}}
            with patch("builtins.open", mock_open(read_data=json.dumps(raw))):
                await runtime._reload_config()
    ...
    async def test_check_config_changes_watchdog_triggers_reload(
        ...
        with (
            ...
            mock_manager = Mock()
            mock_manager.add_transition_callback = Mock()
            mock_manager._get_runtime_config_path = Mock(return_value=temp_config_file)
            mock_manager.current_mode_name = "default"
            mock_manager_class.return_value = mock_manager

            runtime = ModeCortexRuntime(mock_system_config, "test_config", use_watchdog=True)
            runtime.config_path = temp_config_file
