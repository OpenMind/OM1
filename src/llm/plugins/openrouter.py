...
class LlamaLLM(LLM[R]):
    ...
    async def ask(
        ...
        """
        try:
            logging.info(f"Openrouter LLM input: {prompt}")
            logging.info(f"Openrouter LLM messages: {messages}")
            # logging.info(f"Openrouter LLM output model: {self._output_model}")
            ...
            try:
                ...
                logging.info(f"Openrouter output: {parsed_response}")
