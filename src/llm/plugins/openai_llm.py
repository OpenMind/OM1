...
class OpenAILLM(LLM[R]):
    ...
    async def ask(self, prompt: str, messages: T.Optional[T.List[T.Dict[str, str]]] = None) -> T.Optional[R]:
        ...
        try:
            ...
            logging.error(f"OpenAI API error: HTTP {e.status_code} - {e.message}")
            return None
        except openai.APIConnectionError as e:
            logging.error(f"OpenAI connection error: {e.message}")
            return None
        except Exception as e:
            logging.error(f"OpenAI unexpected error: {type(e).__name__}: {e}")
            return None
