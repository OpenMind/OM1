...
class GreetingConversationConnector(
    ...
    def close(self) -> None:
        ...
        Stops the Kokoro TTS provider.
        """
        if self.tts is not None:
            try:
                self.tts.stop()
                logging.info("Greeting Conversation Kokoro TTS provider stopped")
            except Exception as e:
                logging.error(
                    f"Error stopping Greeting Conversation Kokoro TTS provider: {e}"
                )
