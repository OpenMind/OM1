...
class UnitreeG1ReturnToBaseConfig(ActionConfig):
    """
    Configuration for Unitree G1 Return To Base connector.
...
class UnitreeG1ReturnToBaseConnector(
    ActionConnector[UnitreeG1ReturnToBaseConfig, ReturnToBaseInput]
):
    """
    Return to base connector for Unitree G1 robots.
    ...
    def __init__(self, config: UnitreeG1ReturnToBaseConfig):
        """
        Initialize the UnitreeG1ReturnToBaseConnector.
        ...
        logging.info(
            "[ReturnToBaseG1Connector] Initialized. Default base location: '%s'",
    ...
    async def connect(self, output_interface: ReturnToBaseInput) -> None:
        ...
        logging.info("[ReturnToBaseG1Connector] Navigating to base: '%s'", base_label)
        ...
        if loc is None:
            ...
            logging.warning("[ReturnToBaseG1Connector] %s", msg)
            return
        ...
        try:
            ...
            logging.info(
                "[ReturnToBaseG1Connector] Navigation to base '%s' initiated successfully.",
                base_label,
            )
        except Exception as e:
            logging.error(
                "[ReturnToBaseG1Connector] Failed to publish navigation goal to base '%s': %s",
                base_label,
                e,
            )
